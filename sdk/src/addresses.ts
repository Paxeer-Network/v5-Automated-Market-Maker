...
export const PAXEER_ADDRESSES: ChainAddresses = {
...
export function getAddresses(chainId: number): ChainAddresses {
  ...
  if (!config) throw new Error(`Unsupported chain ID: ${chainId}. Supported: ${Object.keys(CHAIN_CONFIG).join(', ')}`);
