...
async function main() {
  ...
  const owner = await (
    await ethers.getContractAt('OwnershipFacet', diamond.target as string)
  ).owner();
  ...
  const feesCollected =
    treasuryBal0After - treasuryBal0Before + (treasuryBal1After - treasuryBal1Before);
  ...
  const results = {
    ...
    contracts: {
      ...
      Token0: token0.target,
      Token1: token1.target,
      ...Object.fromEntries(Object.entries(facets).map(([name, c]) => [name, c.target])),
    },
    poolId,
    passed,
    failed,
