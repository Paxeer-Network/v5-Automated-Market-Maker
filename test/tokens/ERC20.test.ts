...
describe("ERC20", function () {
  ...
  describe("transfer", function () {
    ...
    it("should revert on insufficient balance", async function () {
      ...
      ).to.be.revertedWithCustomError(token, "ERC20_InsufficientBalance");
    });
    ...
    it("should revert on transfer to zero address", async function () {
      ...
      ).to.be.revertedWithCustomError(token, "ERC20_InvalidRecipient");
    });
  });
  ...
  describe("approve and transferFrom", function () {
    it("should approve and allow transferFrom", async function () {
      ...
      expect(await token.allowance(owner.address, alice.address)).to.equal(ethers.parseEther("500"));
      ...
      expect(await token.allowance(owner.address, alice.address)).to.equal(ethers.parseEther("300"));
    });
