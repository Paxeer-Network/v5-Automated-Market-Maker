...
describe("Diamond", function () {
  async function deployDiamondFixture() {
    ...
    return { diamond, diamondCut, loupe, ownership, diamondCutFacet, loupeFacet, ownershipFacet, owner, alice };
  }
  ...
  describe("Ownership", function () {
    ...
    it("should prevent non-owner from transferring", async function () {
      const { ownership, alice } = await loadFixture(deployDiamondFixture);
      await expect(
        ownership.connect(alice).transferOwnership(alice.address)
      ).to.be.reverted;
    });
  });
  ...
  describe("DiamondCut", function () {
    it("should revert when non-owner tries to cut", async function () {
      ...
      await expect(
        diamondCut.connect(alice).diamondCut([], ethers.ZeroAddress, "0x")
      ).to.be.reverted;
    });
  });
