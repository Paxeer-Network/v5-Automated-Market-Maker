...
describe("Facet Integration Tests", function () {
  ...
  async function deployFullDiamondFixture() {
    ...
    const facetCuts = [
      ...
      { facetAddress: ownershipFacet.target, action: 0, functionSelectors: getSelectors(ownershipFacet) },
      ...
      { facetAddress: liquidityFacet.target, action: 0, functionSelectors: getSelectors(liquidityFacet) },
      ...
      { facetAddress: flashLoanFacet.target, action: 0, functionSelectors: getSelectors(flashLoanFacet) },
    ...
    return {
      diamond, pool, swap, liquidity, fee, order, loupe,
      token0, token1, owner, alice, bob, treasury,
      initArgs,
    };
  }
  ...
  describe("PoolFacet", function () {
    ...
    it("should initialize a pool and emit PoolInitialized", async function () {
      const { pool, token0, token1 } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
  ...
  describe("LiquidityFacet", function () {
    it("should add initial liquidity and create position", async function () {
      const { pool, liquidity, token0, token1, owner } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
    ...
    it("should add proportional liquidity on second deposit", async function () {
      const { pool, liquidity, token0, token1, owner, alice } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
    ...
    it("should remove liquidity and return tokens", async function () {
      const { pool, liquidity, token0, token1, owner, diamond } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
  ...
  describe("SwapFacet", function () {
    it("should execute exact-input swap (token0 -> token1)", async function () {
      const { pool, swap, liquidity, token0, token1, owner, alice } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
    ...
    it("should update pool price after swap", async function () {
      const { pool, swap, liquidity, token0, token1, owner, alice } = await loadFixture(deployFullDiamondFixture);
      const { poolId, sqrtPriceX96 } = await createAndInitPool(pool, token0.target as string, token1.target as string);
  ...
  describe("FeeFacet", function () {
    ...
    it("should calculate progressive fee", async function () {
      const { pool, fee, liquidity, token0, token1, owner } = await loadFixture(deployFullDiamondFixture);
      const { poolId } = await createAndInitPool(pool, token0.target as string, token1.target as string);
